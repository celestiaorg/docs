...
<Callout type="info">
Hyperlane uses relayers to forward messages between chains. Relayers are processes that constantly scan for outbound messages on one chain and submit these messages alongside corresponding proofs on the destination chain. This doc covers using Hyperlane to bridge assets and send messages.
</Callout>
