...
1. Celestia publishes blob data and uses data availability sampling (DAS) so light nodes can verify availability by sampling small chunks.
2. Namespaced Merkle trees (NMTs) let each app fetch only its own namespaced data and prove availability.
3. Block producers erasure-code blobs into a _2k × 2k_ matrix, commit to every row and column, and put that root in the header.
4. Light nodes sample within a rolling window; archival nodes (or providers) keep older data retrievable.
5. Celestia is modular: DA and consensus at the base, with execution and settlement layers above.

## Quick links

- [DAS](#data-availability-sampling-das)
- [Namespaced Merkle trees](#namespaced-merkle-trees-nmts)
- [Lifecycle of a transaction](#the-lifecycle-of-a-celestia-app-transaction)
- [Retrievability & pruning](#data-retrievability-and-pruning)
- [Modular vs monolithic](#monolithic-vs-modular-blockchains)
...
into _k × k_ shares, arranged in a _k × k_ matrix, and extended with parity
data into a _2k × 2k_ extended matrix by applying multiple
times Reed-Solomon encoding.

Then, _4k_ separate Merkle roots are computed for the rows and columns
...
block headers, Celestia light nodes need to download the _4k_ intermediate
Merkle roots.

For a block data size of _n²_ bytes, this means that every light node must
download _O(n)_ bytes. Therefore, any improvement in the bandwidth capacity
of Celestia light nodes has a quadratic effect on the throughput of Celestia's
DA layer.

### Fraud proofs of incorrectly extended data

The requirement of downloading the _4k_ intermediate Merkle roots is a
...
where the original data is split into _k_ shares and extended with _k_ additional
shares of parity data. Since the block data commitment is the Merkle root of the
_2k_ resulting data shares, light nodes no longer need to download _O(n)_ bytes to
...
(_i.e._, at least k for a standard encoding and _k × k_ for a
...
Contrastingly, with 2-dimensional Reed-Solomon encoding, only _O(n)_ bytes are
...
[described in the "Celestia's data availability layer" page](/learn/celestia-101/DA),
the commitment is the Merkle root of the _4k_ intermediate Merkle roots
...
- It arranges these shares into a square matrix (row-wise). Note that
  ...
  of size _k × k_ is referred to as the original data.
- It extends the original data to a _2k × 2k_ square matrix using
...
relevant DA metadata, such as the _4k_ intermediate Merkle roots), and
...
- **Use professional archival node or data providers.** It is expected that
  ...
  [network pages](/how-to-guides/mainnet) page, and for specific archival
  node endpoints, refer to the [archival DA RPC endpoints](/how-to-guides/mainnet#archival-da-rpc-endpoints)
  section.
