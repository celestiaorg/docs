...
_Total Fee = Gas Limit x Gas Price_
...
_Gas Limit = FC + sum(i=1..n) SSN(B_i) x SS x GCPBB_

Where:

- _FC_ = Fixed Cost, is a static value (65,000 gas)
- _sum(i=1..n) SSN(B_i)_ = SparseSharesNeeded for the ith Blob, is the number of shares needed for the ith blob in the transaction
- _SS_ = Share Size, is the size of each share
- _GCPBB_ = Gas Cost Per Blob Byte, is a parameter that could potentially be adjusted through the system's governance mechanisms.
