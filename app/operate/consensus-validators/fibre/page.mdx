...
This guide covers Mocha using the celestia-app release
...
`celestia-appd query valaddr providers` lists the hosts of all bonded validators.
Registration persists across restarts, so re-run `set-host` only when the
address changes. The entry is removed automatically if your validator leaves
the active set for good, for example after being jailed and unbonded for more
than 7 days. Register again after rejoining.
