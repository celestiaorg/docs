...
<Tabs items={['Mainnet Beta', 'Mocha']}>
  ...
  <Tabs.Tab>
    ```bash
    PERSISTENT_PEERS=$(curl -sL https://raw.githubusercontent.com/celestiaorg/networks/main/{{constants['mochaChainId']}}/peers.txt | tr '\n' ',')
    echo $PERSISTENT_PEERS
    sed -i.bak -e "s/^persistent_peers *=.*/persistent_peers = \"$PERSISTENT_PEERS\"/" $HOME/.celestia-app/config/config.toml
    ```
  </Tabs.Tab>
...
<Callout type="warning">
Fresh celestia-app v9 nodes default to `min-retain-blocks = 3000`. Any non-zero
value enables block pruning, so archival operators must explicitly set the
value to `0`. Changing the value from `0` to a non-zero value starts pruning the
existing backlog when the node restarts and can temporarily increase sync time.
</Callout>
