...
- Run an archival bridge node (on Mainnet Beta if you are already an active Mainnet Beta validator or on Mocha testnet if not) that is connected and reporting to the Celestia Labs [OTEL collector](/operate/data-availability/metrics) (for new applicants - on testnet, so that we can evaluate performance)
