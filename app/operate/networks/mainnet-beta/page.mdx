...
Because blobs are carried inside the encoded block, the effective block
capacity is the smaller of the square capacity and the encoded-block limit,
so roughly 32 MiB on all networks today.

The following provides an approximation of the blob capacity of the square
size bound alone on Arabica, before applying the 32 MiB encoded-block limit:
