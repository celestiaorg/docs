...
upgrade from `mocha-4`. During the cutover, keep existing `mocha-4`
nodes and data available until `mocha-5` is running successfully.
Existing consensus operators must use a separate home directory for
`mocha-5`, as described below.
</Callout>
