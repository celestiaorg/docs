...
Mocha is planned to restart as a new chain with the chain ID `mocha-5`.
The new chain starts at height 1; it is not an in-place upgrade from
`mocha-4`. Keep existing `mocha-4` nodes and data available until
`mocha-5` is running successfully.
</Callout>
