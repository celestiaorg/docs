...
<Callout type="info">
Bridge nodes run in pruned mode by default and do not require a consensus
endpoint with full history. For an archival bridge node, start with `--archival`
and find [an archive endpoint on the community dashboard](https://celestia-tools.brightlystake.com/)
or run your own consensus node with pruning disabled.
</Callout>
