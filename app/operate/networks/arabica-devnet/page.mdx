...
nodes can use a non-archival consensus endpoint; older headers and data are
retrieved from the peer-to-peer network. To retain full history locally, start
the bridge node with `--archival`.
</Callout>
