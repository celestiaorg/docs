...
<Callout type="info">
Bridge nodes automatically route header requests between the connected consensus
node and the peer-to-peer network. No additional routing configuration is
required.
</Callout>
...
`--archival` is a start-time option. Include it every time the node starts,
including in a systemd service or another process manager.
</Callout>
