...
<Callout type="info">
Bridge nodes automatically route recent header requests to the connected
consensus node and older requests to the peer-to-peer network. No additional
...
<Tabs items={['Pruned (default)', 'Archival']}>
  <Tabs.Tab>
    ```sh
...
Choose archival mode before the node's first start and include `--archival`
every time it starts, including in a systemd service or another process manager.
A store that has already pruned data cannot switch back to archival mode; use a
fresh store to sync the full history from genesis.
</Callout>
