...
1. Set [the network](/operate/networks/overview) you would like to run your node on:
...
3. Set an RPC endpoint for either [Mainnet Beta](/operate/networks/mainnet-beta#integrations),
   ...
   <Tabs items={['Mainnet Beta', 'Mocha', 'Arabica']}>
     <Tabs.Tab>
       ...
       docker run -e NODE_TYPE=$NODE_TYPE -e P2P_NETWORK=$NETWORK \
           ghcr.io/celestiaorg/celestia-node:v0.28.4 \
     ...
     <Tabs.Tab>
       ```bash
       docker run -e NODE_TYPE=$NODE_TYPE -e P2P_NETWORK=$NETWORK \
           ghcr.io/celestiaorg/celestia-node:v0.28.4-arabica \
           celestia $NODE_TYPE start --core.ip $RPC_URL --core.port $RPC_PORT --p2p.network $NETWORK
       ```
     </Tabs.Tab>
   </Tabs>
...
[celestia-node tutorial](/operate/data-availability/light-node#start-the-light-node) page. Refer to
