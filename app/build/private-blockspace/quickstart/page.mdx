...
To use **private blockspace**, you’ll need access to a hosted **Celestia Data Availability (DA) + Consensus Node**.  
The easiest option is **[QuickNode’s Celestia service](https://www.quicknode.com/docs/celestia)**, which provides ready-to-use mainnet and Mocha testnet endpoints.
...
<Callout type="info">
You’ll need to create a QuickNode plan before spinning up your endpoint.  
They offer a **free 30-day trial**, which is enough for this quickstart.
</Callout>

3. Copy the generated endpoint URL (it looks like this):
   ```
...
docker run --name pda-proxy --rm -it   --platform=linux/amd64   --env-file ./.env   -v "$PDA_DB_PATH:$PDA_DB_PATH"   -v "./tls:/app/tls"   -p 26657:26657   ghcr.io/celestiaorg/pda-proxy:latest
```
...
docker run --name pda-proxy --rm -it   --env-file ./.env   -v "$PDA_DB_PATH:$PDA_DB_PATH"   -v "./tls:/app/tls"   -p 26657:26657   ghcr.io/celestiaorg/pda-proxy:latest
```
