...
More information on the RPC that is exposed by a Celestia light node can be found [in the Node API docs](/build/rpc/node-api/?version=v0.28.4). Additionally, if you need more information on how to run a light node, you can [check out the documentation](/operate/data-availability/light-node/quickstart).
