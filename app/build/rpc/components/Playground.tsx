...
const sendRequest = async (
  ...
  return new Promise((resolve, reject) => {
    ...
    const wsUrl = hostname === '' ? 'ws://localhost:26658' : `ws://${hostname}`;
    ...
    const connectionTimeout = setTimeout(() => {
      if (!connectionEstablished && !connectionClosed) {
        ...
        reject(new Error(`Connection timeout: Unable to connect to ${wsUrl}. Make sure your Celestia node is running.`));
      }
...
const sendRequestClickHandler = async ({
  currentRequest,
  setNotification,
  setCurrentResponse,
  setCurrentTab,
  ...
  try {
    const data = await sendRequest(currentRequest, hostname);
              ...
              >
                ...
                <div style={{ marginTop: '1.25rem', display: 'flex', flexDirection: 'row-reverse', gap: '0.75rem' }}>
                  <button
                    ...
                    onClick={() => sendRequestClickHandler({ currentRequest, setNotification, setCurrentResponse, setCurrentTab, hostname })}
                    data-autofocus
                  >
                    Send Request
                  </button>
