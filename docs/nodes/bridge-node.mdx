...
```sh
celestia bridge init --core.ip https://limani.celestia-devops.dev/ --core.grpc.port 9090
```
...
```sh
celestia bridge init --core.remote https://rpc-mocha.pops.one:9090
```
...
```sh
celestia bridge start --core.remote https://rpc-mocha.pops.one:26657
```
...
```sh
celestia bridge start --core.remote <ip>:<port> --keyring.accname <name_of_custom_key>
```
