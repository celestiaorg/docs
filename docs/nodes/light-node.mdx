...
```sh
celestia light start --core.ip <ip-address>:<port>
```

For ports:

> NOTE: The `--core.ip` port defaults to 9090, so if you do not specify
...
If you need a list of RPC endpoints to connect to, you can check from the list [here](./arabica-devnet)

For example, your command might look something like this:

```sh
celestia light start --core.ip https://limani.celestia-devops.dev --core.grpc.port 9090 --gateway --gateway.addr 0.0.0.0 --gateway.port 26659
```
...
```sh
celestia light start --core.ip <ip-address>:<port> --keyring.accname <key-name>
```
...
```sh
celestia light start --core.ip <ip-address>:<port> --keyring.accname <name_of_custom_key>
```
