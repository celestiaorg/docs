...
An example init command for Arabica will look similar to below:

````mdx-code-block
<pre><code>
docker run -e NODE_TYPE=light -e P2P_NETWORK=arabica -v $HOME/my-node-store:/home/celestia ghcr.io/celestiaorg/celestia-node:{arabicaVersions['node-latest-tag']} celestia light init --p2p.network arabica
</code></pre>
````
...
````mdx-code-block
<pre><code>
docker run -e NODE_TYPE=light -e P2P_NETWORK=arabica -v $HOME/my-node-store:/home/celestia ghcr.io/celestiaorg/celestia-node:{arabicaVersions['node-latest-tag']} celestia light start --core.ip consensus-full-arabica-8.celestia-arabica.com --gateway --gateway.addr 127.0.0.1 --gateway.port 26659 --p2p.network arabica
</code></pre>
````
