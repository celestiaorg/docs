...
--gateway --gateway.addr 127.0.0.1 --gateway.port 26659 --p2p.network mocha-3
```
...
services:
  celestia:
    ...
    environment:
      - NODE_TYPE=light
    command: celestia light start --core.ip rpc-mocha.pops.one --gateway.deprecated-endpoints --gateway --gateway.addr 127.0.0.1 --gateway.port 26659 --p2p.network mocha-3 --keyring.accname my_celes_key
    volumes:
