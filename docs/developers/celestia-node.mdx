...
can the user please re-initialise their node via celestia light init -- this is likely due to an old config being present. Re-initialisation will reset the config.
...
```sh
rm -rf ~./celestia-light-arabica && celestia light init
```
...
```sh
@ ~ % rm -rf ./celestia-light-arabica && celestia light init
2022-11-01T22:23:19.581+0100	INFO	node	nodebuilder/init.go:20	Initializing Light Node Store over '/Users/joshstein/.celestia-light-arabica'
2022-11-01T22:23:19.582+0100	INFO	node	nodebuilder/init.go:51	Saving config	{"path": "/Users/joshstein/.celestia-light-arabica/config.toml"}
