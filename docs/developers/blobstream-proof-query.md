---
sidebar_label: Querying the Blobstream proofs
...
Make sure to have access to a Celestia consensus full node RPC endpoint.

> **_NOTE:_** The node doesn't need to be a validating node in order for the proofs to be queried. Being a full node is enough.
...
> **_NOTE:_** The values are base64 encoded. For these to be usable with the solidity smart contract, they need to be converted to `bytes32`. Check the next section for more information.
...
> **_NOTE:_** If any of the fields is empty, then it will not be in the response. For example, if the `start` field is `0`, it will be omitted in the response.
...
Which are the roots of the rows where the shares containing the Rollup data are localised. These can be taken from the `prove_shares` query response:
...
> **_NOTE:_** The above proofs constructions are done in Solidity, different gymnastics will be required for different programming languages. Reach out to the team if more information is needed.
