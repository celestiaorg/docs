...
The format for RPC methods is as follows:
...
* an RPC address
* the location of your [node's store](#node-store) OR the
[authentication token](#auth-token-) for your node

### Setup

When running `celestia [module] [method] [...args]` commands,
you will need to set either the [node store](#node-store)
or the [authentication token](#auth-token-).

The RPC CLI handles these flags in the following order:

1. If user passes auth token, use that
2. If user doesn't pass auth token, check node store flag, create token from
node store, and use that
...
you will need to set your node store. This will
allow you to interact with the API without having
to set an authentication token.

To set your node store for `mocha-4`, you can use the following command:
...
This section will show you how to set your authentication token.

The `--token string` flag sets the authentication token,
otherwise it will read from the node's store.
...
To set the log-level when the node is running, you can use the following command:
...
```bash
celestia rpc blob get [block height] [namespace in hex] [commitment from output above] [node store or auth]
```
