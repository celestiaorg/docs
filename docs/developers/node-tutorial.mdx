...
```sh
celestia light start --core.ip rpc-arabica-9.consensus.celestia-arabica.com --p2p.network arabica
```
...
`272667` which we will use for the next command.
...
Here is what an example of the format of the `GetAll` command looks like:

```bash
celestia rpc blob GetAll [block height] [namespace in hex]
```

Here is an example command to retrieve the data from above, on `arabica-9`:

```bash
celestia rpc blob GetAll 272667 0x42690c204d39600fddd3
```
...
```bash
celestia rpc blob GetAll 272667 0x42690c204d39600fddd3 --print-request
```
...
```bash
{"jsonrpc":"2.0","result":[{"namespace":"AAAAAAAAAAAAAAAAAAAAAAAAAEJpDCBNOWAP3dM=","data":"8fIMqAB+kQo7+LLmHaDya8oH73hxem6lQWX1","share_version":0,"commitment":"R4DRTAENQ7fGawPbt8aMse1+YReCZYg87xZIZf2fAxc="}],"id":1}
```
