...
In this tutorial, you will cover how to use the Celestia Node API to submit and
...
so you must install Golang to build and run them.
...
Now you need to add the `/usr/local/go/bin` directory to `$PATH`:
...
Now, let's query your node for the balance of its default account
(which is the account associated with the `developer` key you generated earlier):
...
Here you will get the header from Block 1:
...
In this example, you will be submitting a PayForData
...
You use the following `namespace_id` of `0000010000000100` and
...
useful Golang Playground you created [here](https://go.dev/play/p/7ltvaj8lhRl).
...
`2452` which you will use for the next command.
...
the block height you got was 589 which you will use for the following command.
