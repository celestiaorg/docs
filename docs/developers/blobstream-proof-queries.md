...
- Access to a Celestia [consensus full node](../../nodes/consensus-node)
...
This is the shares proof to the row roots. These can contain multiple proofs if the shares
containing the blob span across multiple rows. To construct them, we will use the result of
the [transaction inclusion proof](#2-transaction-inclusion-proof) section:
...
So, we construct a `NamespaceNode` via taking the values from the `nodes` field in the
query response, we convert them from base64 to `hex`, then we use the following mapping:
...
- `proof`: the `BinaryMerkleProof` of the data root tuple to the data root tuple root.
