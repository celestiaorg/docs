...
```bash
docker logs --follow ghcr.io/celestiaorg/celestia-node:sha-$sha
```
...
```bash
docker stop ghcr.io/celestiaorg/celestia-node:sha-$sha
```
...
```bash
docker rm ghcr.io/celestiaorg/celestia-node:sha-$sha
````
