...
This is the default for the OP Stack + Celestia repository. 
...
In order to use a light node, you will need to open up the
`optimism/ops-bedrock/docker-compose.yml` file in your text editior
and comment out lines 16-28, and uncomment lines 37-50. You will also need
to comment out the `depends _on:` section on lines 165-167.
...
cd optimism
git checkout op-node/v1.0.6-rc.1-celestia-node-v0.9.2
```
