...
You may need to give permissions to your keys in order to do this.
Here are some example commands being ran from the `$HOME/optimism/`
directory:

```bash
chmod go-rwx op-celestia/blockspacerace-data/keys/OAZHALLLMV4Q
chmod go-rwx op-celestia/blockspacerace-data/keys/NJ3XILLTMVRXEZLUFZVHO5A
chmod go-rwx op-celestia/blockspacerace-data/keys
```
:::
