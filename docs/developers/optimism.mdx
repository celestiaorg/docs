...
software is not yet released, but available on `v0.9.6-alpha.2`.

When installing `celestia-node`, you can use `git checkout v0.9.6-alpha.2`
before building the binary.

Next, in order to allow your light node to post
and retrieve data without errors, you will need to change the `UseShareExchange`
