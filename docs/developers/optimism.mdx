...
The current version of Celestia that is compatible with
software is `v0.9.5` on the `blockspacerace` testnet.

In order to allow your light node to post
and retrieve data without errors, you will need to change `UseShareExchange`
to `false` in the `$HOME/.celestia-light-blockspacerace-0/config.toml` file.
:::
...
in the `$HOME/.celestia-light-blockspacerace-0` directory. This is the
