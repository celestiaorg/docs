...
function withVocsMdxCompatibility(content, sourcePath) {
  return withVocsFrontmatter(content)
    .replace(/(from\s+['"])(\.[^'"]+)(['"])/g, (_match, before, specifier, after) => {
      ...
      return `${before}@/${toPosix(path.relative(rootDir, resolved))}${after}`
    })
    .replaceAll(
...
async function copyAppTree(currentDir = appDir) {
  ...
  for (const entry of entries) {
    ...
    const targetPath = path.join(generatedPagesDir, path.dirname(relativePath), 'index.mdx')
    await mkdir(path.dirname(targetPath), { recursive: true })
    generatedFiles.add(toPosix(path.relative(generatedPagesDir, targetPath)))
    const content = await readFile(sourcePath, 'utf8')
    await writeFile(targetPath, withVocsMdxCompatibility(content, sourcePath))
  }
}
